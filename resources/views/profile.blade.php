    ...
    <div class="grid items-start gap-6 lg:grid-cols-[3fr_2fr]">
        <x-card>
            ...
            <form class="space-y-6" method="POST" enctype="multipart/form-data" action="{{ route('profile.update') }}">
                @csrf @method('PATCH')<x-avatar :user="$user" />
                <x-input name="avatar" label="Profile picture" type="file"
                ...
                <div>
                    ...
                    <textarea id="bio" name="bio" rows="3" maxlength="500"
                        class="w-full rounded-lg border border-slate-300 px-3 py-2.5">{{ old('bio', $user->bio) }}</textarea>
                </div>
        ...
        <x-card>
            ...
            <form class="space-y-5" method="POST" action="{{ route('profile.password') }}">
                @csrf @method('PUT')<x-input name="current_password" label="Current password" type="password"
                    required autocomplete="current-password" />
                <x-input name="password" label="New password" type="password" required minlength="8"
