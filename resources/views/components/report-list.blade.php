        ...
        <article class="flex flex-wrap items-center justify-between gap-4 py-5">
            <div>
                ...
                @if ($report->status === 'failed')
                    <p class="mt-2 text-sm text-red-700">This report failed. Please request a new report.</p>
                @endif
            </div>
            <div class="flex items-center gap-4">
                <x-status-badge :status="$report->status" />
                @if ($report->status === 'completed')
                    <a class="text-brand text-sm font-medium" href="{{ route('reports.download',$report) }}"
                        >Download CSV<span class="sr-only"> report {{ $report->id }}</span>
                    </a>
                @endif
            </div>
        </article>
