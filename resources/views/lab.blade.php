<x-layout title="Lab">
    ...
    <div class="grid gap-6 md:grid-cols-2">
            ...
            <dl class="space-y-4">
                @foreach (['version' => 'Version', 'commit' => 'Commit', 'instance' => 'Instance'] as $key => $label)
                    <div class="flex justify-between gap-4">
                        ...
                        <dd class="break-all font-mono text-sm">{{ config('hub.' . $key) }}</dd>
                    </div>
                @endforeach
            </dl>
        </x-card>
        <x-card>
            ...
            @foreach ($checks as $name => $ok)
                <div class="flex justify-between py-2">
                    ...
                    <span
                        class="text-sm font-medium {{ $ok ? 'text-brand' : 'text-red-700' }}">{{ $ok ? 'Available' : 'Unavailable' }}</span>
                </div>
            @endforeach
        </x-card>
        <x-card>
            <h2 class="mb-4 text-lg font-semibold">Scheduler heartbeat</h2>
            <p class="font-mono text-sm">{{ $heartbeat?->last_ran_at->toIso8601String() ?? 'No heartbeat recorded' }}
            </p>
        ...
        <x-card>
            ...
            <p class="text-sm leading-relaxed text-slate-500">These statuses are recorded by report jobs. Pending means
                a worker has not started the job. Backend connectivity alone does not confirm a running worker.</p>
        </x-card>
    </div>
    <x-card class="mt-6">
